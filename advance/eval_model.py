...
def init_model(args):
    ...
    if args.load == 0:
        ...
        model = MainForCausalLM(MainConfig(
        ...
        transformers_model_path = './Main2'
        tokenizer = AutoTokenizer.from_pretrained(transformers_model_path)
        model = AutoModelForCausalLM.from_pretrained(transformers_model_path, trust_remote_code=True)
    # print(f'Main模型参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)')
...
def main():
    parser = argparse.ArgumentParser(description="Chat with Main")
    ...
    # Main2-moe (145M)：(hidden_size=640, num_hidden_layers=8, use_moe=True)
    # Main2-Small (26M)：(hidden_size=512, num_hidden_layers=8)
    # Main2 (104M)：(hidden_size=768, num_hidden_layers=16)
